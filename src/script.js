...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Information content
  const infoContent = {
    option1: `<div class="info-content"> <h2 class="info-content-heading">Password Security</h2>This script will assess various password policies such as password length, history, complexity, and more, based on the CIS Benchmark recommendations <br> Maximum password age (days):                     
<br>Lockout duration (minutes):                        
<br>Lockout observation window (minutes):              
<br>Required configurations:
<br>Maximum password age (days):                        
<br>Lockout duration (minutes):                         
<br>Lockout observation window (minutes):               
</div>`,
  ...
  const defaultContent = `<div class="default-content"><h2 class="info-content-heading">Main Heading</h2>Please select an option to see the information.To check The system Configuration and compare it with cis benchmark to know the Vulnerabilities in yous system</div>`;
